...
func (c *Controller) Solve(ctx context.Context, req *controlapi.SolveRequest) (*controlapi.SolveResponse, error) {
	...
	if req.Exporter != "" {
		...
		if c.opt.UseSnapshotter {
			if req.Exporter == mobyexporter.Moby {
			...
			if req.ExporterAttrs != nil {
				...
				req.ExporterAttrs["name"] = strings.Join(reposAndTags, ",")
...
// to a slice of repoAndTag. It removes duplicates and validates each repoName
// and tag to not contain a digest.
