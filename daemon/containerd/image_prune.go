...
func (i *ImageService) pruneUnused(ctx context.Context, filterFunc imageFilterFunc, danglingOnly bool) (*types.ImagesPruneReport, error) {
	...
	for _, img := range imagesToPrune {
		...
		err = containerdimages.Walk(ctx, containerdimages.Handlers(
			i.presentChildrenHandler(containerdimages.HandlerFunc(func(_ context.Context, desc ocispec.Descriptor) ([]ocispec.Descriptor, error) {
...
// and calls specified handler on every existing content.
func (i *ImageService) presentChildrenHandler(h containerdimages.HandlerFunc) containerdimages.HandlerFunc {
	store := i.client.ContentStore()
