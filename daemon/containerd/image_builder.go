...
func (i *ImageService) GetImageAndReleasableLayer(ctx context.Context, refOrID string, opts backend.GetImageAndLayerOptions) (builder.Image, builder.ROLayer, error) {
	...
	if opts.PullOption != backend.PullOptionForcePull {
		// todo: certainly we don't always need to call i.GetImage AND i.GetContainerdImage
		img, err := i.GetImage(ctx, refOrID, image.GetImageOpts{Platform: opts.Platform})
		if err != nil && opts.PullOption == backend.PullOptionNoPull {
			return nil, nil, err
		}
		dimg, err := i.GetContainerdImage(ctx, refOrID, opts.Platform)
		...
		if img != nil {
			...
			layer, err := newROLayerForImage(ctx, &dimg, i, opts, refOrID, opts.Platform)
			if err != nil {
				return nil, nil, err
			}
	...
	img, c8dImg, err := i.pullForBuilder(ctx, refOrID, opts.AuthConfig, opts.Output, opts.Platform)
	if err != nil {
		return nil, nil, err
	}

	layer, err := newROLayerForImage(ctx, c8dImg, i, opts, refOrID, opts.Platform)
	if err != nil {
		return nil, nil, err
	}
...
func (i *ImageService) pullForBuilder(ctx context.Context, name string, authConfigs map[string]registry.AuthConfig, output io.Writer, platform *ocispec.Platform) (*dimage.Image, *containerdimages.Image, error) {
...
Please notify the image author to correct the configuration.`,
	...
	dimg, err := i.GetContainerdImage(ctx, name, platform)
	if err != nil {
		return nil, nil, err
	}
...
func newROLayerForImage(ctx context.Context, img *containerdimages.Image, i *ImageService, opts backend.GetImageAndLayerOptions, refOrID string, platform *ocispec.Platform) (builder.ROLayer, error) {
	if img == nil {
	...
	diffIDs, err := img.RootFS(ctx, i.client.ContentStore(), platMatcher)
	if err != nil {
		return nil, err
	}
	...
	return &rolayer{
		...
		c:                  i.client,
		snapshotter:        i.snapshotter,
		diffID:             identity.ChainID(diffIDs),
		root:               root,
		contentStoreDigest: "",
	}, nil
}
...
func (i *ImageService) CreateImage(ctx context.Context, config []byte, parent string, layerDigest digest.Digest) (builder.Image, error) {
	...
	if parent != "" {
		parentImageC8d, err := i.resolveImage(ctx, parent, nil)
		if err != nil {
			return nil, err
		}
		parentImageManifest, err := containerdimages.Manifest(ctx, i.client.ContentStore(), parentImageC8d.Target, platforms.Default())
	...
	commitManifestDesc, configDigest, err := writeContentsForImage(ctx, i.snapshotter, i.client.ContentStore(), ociImgToCreate, layers)
	if err != nil {
		return nil, err
	}
...
func (rw *rwlayer) Commit() (builder.ROLayer, error) {
	ctx := context.TODO()
	snapshotter := rw.c.SnapshotService(rw.s)

	err := snapshotter.Commit(context.TODO(), rw.key, rw.parent)
	if err != nil && !cerrdefs.IsAlreadyExists(err) {
		return nil, err
	}

	// make a diff for the committed snapshot so we can
	// use the info from the content store to get the DiffID
	differ := rw.c.DiffService()
	desc, err := rootfs.CreateDiff(ctx, rw.key, snapshotter, differ)
	if err != nil {
		return nil, err
	}
